...
def make_dir(urls):
    ...
    if not os.path.exists(t):
        os.mkdir(t)
    folder_name = datetime.datetime.now().strftime("%d-%m-%Y %H-%M-%S")#%H_%M_%S-%Y_%m_%d
    folder_name = os.path.join(t,folder_name)
    if not os.path.exists(folder_name):
        os.mkdir(folder_name)
    folders = []
    for k in range(1,len(urls)+1):
        names = 'image'+str(k)
        main_path = os.path.join(folder_name,names)
...
def refine_data(data,folder_name):
    ...
    main_data['Matches'] = matches_imgs
...
def image_scrapper(urls):
    #mkdir
    folders,urls_list = make_dir(urls)
